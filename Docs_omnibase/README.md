...
To run the serial communication node (assuming the STM32 is already programmed and connected via USB to port /dev/ttyACM0, check port with 'sudo dmesg | grep tty'):

```bash
ros2 run serial_comm serial_communication 
```

> 📝 **Format note**: The STM32 currently expects a string of space-separated floats containing the desired pose `[x, y, phi, d, r]` followed by 21 PID gains, terminated by `\r\n`. This will be changed when RX-TX different "modes" state machine is implemented.
